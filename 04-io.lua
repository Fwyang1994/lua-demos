...
其中，第一个返回值是一个布尔类型，当为 true 时表示程序成功运行完成；
第二个返回值是一个字符串，当为"exit"时表示程序正常运行结束，当为"signal"时表示因信号而中断；
第三个返回值是返回状态（若该程序正常终结）或者终结该程序的信号代码。
]]

--[[
另一个非常有用的函数是io.popen。  同函数os.execute一样，该函数运行一条系统命令，
但该函数还可以重定向命令的输入/输出，从而使得程序可以向命令中写入或从命令的输出中读取。
]]
